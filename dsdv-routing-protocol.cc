...
{
  NS_LOG_FUNCTION (this << p->GetUid() << p->GetSize() <<header.GetSource() <<header.GetDestination() <<header << (oif ? oif->GetIfIndex () : 0));
                                      ...
                                      ErrorCallback ecb)
{
  ...
  if (result)
    {
      NS_LOG_LOGIC ("Added packet " << p->GetUid () << " " <<  p->GetSize() << " to queue.");
    }
}
...
RoutingProtocol::RouteInput (Ptr<const Packet> p,
                             ...
                             ErrorCallback ecb)
{
  //m_ipv4 = pointer dari main_adddress & main_address = alamat node dirujuk oleh routeinput
  ...
  if (m_socketAddresses.empty ())
    ...
    NS_LOG_LOGIC (node << " nodes " 
                  ...
                  << source->GetRemainingEnergy() << " = sisa energi " 
                  << "\n"
    );
  ...
  //iif = merupakan index dari interface
  ...
  for (std::map<Ptr<Socket>, Ipv4InterfaceAddress>::const_iterator j =
    ...
    {
      Ptr<Socket> socket_node = j->first;
      Ipv4InterfaceAddress iface = j->second;
      NS_LOG_LOGIC( socket_node->GetNode() <<  " " << socket_node->GetNode()->GetId() << " " 
                    << iface << " "  << iface.GetLocal()  << " IFACE"
      );

      if (origin == iface.GetLocal ()) //berguna untuk PENGECEKAN LOCAL = SUMBER NODE ?
        {
          NS_LOG_LOGIC(iface.GetLocal() << " IFACELOCAL");
          return true;
        }
    }
              ...
              if (lcb.IsNull () == false)
                {
                  ...
                  lcb(p, header, iif);
                  NS_LOG_LOGIC(p->GetSize()  << " "<< p->GetUid() << " " << header << " " << iif);
              ...
              if (header.GetTtl () > 1)
                {
                  ...
                  if (m_routingTable.LookupRoute (dst,toBroadcast,true))
                    ...
                    {
                      NS_LOG_LOGIC ("No route to forward. Drop packet " << p->GetUid ());
                    }
                }
              return true;
            }
  ...
  //menentukan apakah alamat yang dituju sesuai dengan alamat destination dan interface yang digunakan pada saat forward data
      ...
      if (lcb.IsNull () == false)
        {
          NS_LOG_LOGIC ("Unicast local delivery to " << dst << " from " << origin  );
             if (source->GetRemainingEnergy() > 500.0){
                NS_LOG_LOGIC("ENERGY > 500, deliver" << " size payload =  "<< p->GetSize()  << " id packet= " << p->GetUid() << 
              ...
              if((p->GetSize() - 8) == 1024){
              NS_LOG_LOGIC("ENERGY < 500, not deliver " << " size payload =  "<< p->GetSize()  << " id packet= " << p->GetUid() << 
                           " Node: " <<node->GetId() << " Energy: " <<source->GetRemainingEnergy());
              }else{
                NS_LOG_LOGIC("ENERGY < 500, deliver  " << " size payload =  "<< p->GetSize()  << " id packet= " << p->GetUid() << 
                            " Node: " <<node->GetId() << " Energy: " <<source->GetRemainingEnergy() << 
	    ...
	    ///START KODE PROGRAM TANPA PEREKAYASAAN TRAFFIC
            //  NS_LOG_LOGIC(" size payload =  "<< p->GetSize()  << " id packet= " << p->GetUid() << 
                            // " Node: " <<node->GetId() << " Energy: " <<source->GetRemainingEnergy()<< 
                            // " from: " << origin  << " to: " << dst );        
            //  lcb (p, header, iif);
      }
      ...
      if (m_routingTable.LookupRoute (toDst.GetNextHop (),ne))
        {
          ...
          NS_LOG_LOGIC (m_mainAddress << " is forwarding packet " << p->GetUid ()<< " " << p->GetSize()
                                      << " to " << dst
                                      << " from " << header.GetSource () // sama cari node source
                                      << " via nexthop neighbor " << toDst.GetNextHop ());

            if (source->GetRemainingEnergy() > 500.0){
              NS_LOG_LOGIC("PACKET FORWARD" << " size payload =  "<< p->GetSize()  << " id packet= " << p->GetUid() <<
                           " Node: " <<node->GetId() << " Energy: " <<source->GetRemainingEnergy());
              ucb (route,p,header);
              return true;
            }
              ...
              if((p->GetSize()- 8) == 1024){
                  NS_LOG_LOGIC("DROP PACKET FORWARD "<< p->GetSize() << " " << p->GetUid()<< 
                               " Node: " <<node->GetId() << " Energy: " <<source->GetRemainingEnergy());
              }
              else {
  ...
  NS_LOG_LOGIC ("Drop packet " << p->GetUid ()<< " size: " << p->GetSize() << " from " << header.GetSource()
...
{
  NS_LOG_LOGIC (m_mainAddress << " drop packet " << packet->GetUid () << " to "
  ...
  if (EnableWST)
    {
      if (mainrt.GetSettlingTime () == Seconds (0))
        {
          NS_LOG_LOGIC("RETURN SETTLING TIME  = 0");
          return Seconds (0);
        }
      else
        {
